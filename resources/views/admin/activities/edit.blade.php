        ...
        <section class="section">
            ...
            <div class="page-header">
                ...
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{trans('update_activity')}} #{{ $activity->id }}</li>
                </ol>
            </div>
            <!--page-header closed-->
                                ...
                                <form action="{{ route('admin.activities.update', ['activity' => $activity]) }}" method="post" enctype="multipart/form-data">
                                        ...
                                        @foreach(config()->get('app.locales') as $lang => $language)
                                            <div class="tab-pane fade {{ $lang == app()->getLocale() ? 'active show': ''}}" id="lang-{{ $lang }}" role="tabpanel" aria-labelledby="home-tab2">
                                                <div class="form-group col-md-4">
                                                    ...
                                                    <input type="text" class="form-control" name="{{ $lang }}[name]" value="{{ $activity->translate($lang)->name }}" id="{{ $lang }}[name]">
                                                </div>

                                                <div class="form-group col-md-4">
                                                   <label for="description">{{trans('description')}} *</label>
                                                    <textarea rows="6" class="form-control" name="{{ $lang }}[description]"id="{{ $lang }}[description]">{{ $activity->translate($lang)->description }}</textarea>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
                                    ...
                                    <div class="form-group col-md-3">
                                        ...
                                        <label class="custom-switch">
                                            ...
                                            <img id="upload_img" src="{{ asset($activity->image) }}">
                                            <input type="hidden" name="image" value="{{ asset($activity->image) }}">
                                        </label>
                                    </div>

                                    <div class="form-group col-md-3">
                                        <label class="custom-switch">
                                            <input type="checkbox" name="active" value="1" {{ $activity->active ? 'checked' : '' }} class="custom-switch-input">
