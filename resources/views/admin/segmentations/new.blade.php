                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-header">
                                <h4>{{trans('new_segmentation')}}</h4>
                            </div>
                            <div class="card-body">
                                @include('admin.errors')

                                @section("open-form-tag")
                                    <form action="{{ route('admin.segmentations.store') }}" method="post">
                                @show
                                    ...
                                    <div class="form-group col-md-4">
                                        <div class="form-group overflow-hidden">
                                            <label for="class">{{trans('zone_class')}}</label>
                                            <select name="location_class"  class="form-control select2 w-100">
                                                <option value="">{{ trans('select_zone_class') }}</option>
                                                @foreach(ClassTypes::getList() as $key => $value)
                                                    <option value="{{ $key }}" {{ old('location_class') == $key? 'selected': null }}>{{ $value }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                        ...
                                        <div class="form-group overflow-hidden">
                                            ...
                                            <select name="orders_category"  class="form-control select2 w-100" id="orders_category" >
                                                ...
                                                @foreach($categories as $category)
                                                    <option value="{{ $category->id }}" {{ old("orders_category") == $category->id ? "selected":null }}>{{ $category->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    <div class="form-group col-md-4">
                                        <div class="form-group overflow-hidden">
                                            ...
                                            <select name="orders_wish_list_category"  class="form-control select2 w-100" id="orders_wish_list_category" >
                                                ...
                                                @foreach($categories as $category)
                                                    <option value="{{ $category->id }}" {{ old("orders_wish_list_category") == $category->id ? "selected":null }}>{{ $category->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    <div class="form-group col-md-4">
                                        <div class="form-group overflow-hidden">
                                            ...
                                            <input type="number" min="1" class="form-control" name="orders_amount" id="orders_amount" value="{{ old('orders_amount') }}">
                                        </div>
                                    </div>
                                    <div class="form-group col-md-4">
                                        <div class="form-group overflow-hidden">
                                            <label for="users_number">{{trans('total_users')}}</label>
                                            <input type="number" min="1" class="form-control" name="users_number" id="users_number" value="{{ old('users_number') }}">
                                        </div>
                                    </div>
                                    <div class="form-group col-md-4">
                                        <div class="form-group overflow-hidden">
                                            <label for="weeks_number">{{trans('weeks_number')}}</label>
                                            <input type="number" min="1" class="form-control" name="weeks_number" id="weeks_number" value="{{ old('weeks_number') }}">
                                        </div>
                                    </div>
