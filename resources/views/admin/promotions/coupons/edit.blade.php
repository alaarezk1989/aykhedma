    ...
    <section class="section">
        ...
        <div class="page-header">
            ...
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">{{ trans('update_coupon') }} #{{ $coupon->id }}</li>
            </ol>
        </div>
        <!--page-header closed-->
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            <h4>{{ trans('update_coupon') }}</h4>
                        </div>
                        <div class="card-body">
                            @include('admin.errors')
                            <form action="{{ route('admin.coupons.update', ['coupon' => $coupon]) }}" method="POST" enctype="multipart/form-data" autocomplete="off" >
                                @method('PUT')
                                @csrf
                                    ...
                                    @foreach(config()->get('app.locales') as $lang => $language)
                                        <div class="tab-pane fade {{ $lang == app()->getLocale() ? 'active show': ''}}" id="lang-{{ $lang }}" role="tabpanel" aria-labelledby="home-tab2">
                                            <div class="form-group col-md-4">
                                                    ...
                                                    <input type="text"  name="{{ $lang }}[title]" class="form-control" value="{{ old($lang.".name", $coupon->translate($lang)->title)}}" id="{{ $lang }}[title]">
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                                <div class="form-group col-md-4">
                                    ...
                                    <select name="vendor_id" id="vendor_id"  class="form-control select2 w-100">
                                        ...
                                        @foreach($vendors as $vendor)
                                            <option value="{{$vendor->id}}" {{ $coupon->vendor_id == $vendor->id ? "selected": null }} >{{$vendor->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group col-md-4">
                                    ...
                                    <select name="branch_id" class="branch_id branch form-control select2 w-100" value="{{old('branch_id')}}" >
                                        ...
                                        @foreach($branches as $branch)
                                            <option value="{{$branch->id}}" {{ $coupon->branch_id == $branch->id ? "selected": null }} >{{$branch->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
...
{{--                                            <option value="{{$activity->id}}" {{ old("activity_id",$coupon->activity_id) == $activity->id ? "selected":null}}>{{$activity->name}}</option>--}}
{{--                                        @endforeach--}}
{{--                                    </select>--}}
{{--                                </div>--}}
                                ...
                                <div class="form-group col-md-4">
                                    ...
                                    <select name="type" id="type"  value="{{old('type')}}" class="form-control select2 w-100">
                                        ...
                                        @foreach($types as $key => $value)
                                            <option value="{{$key}}" {{ old("type",$coupon->type) == $key ? "selected":null}} >{{$value}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group col-md-4">
                                    <label for="value">{{trans('value')}} *</label>
                                    <input type="number" name="value" value="{{$coupon->value}}"  class="form-control" id="value" min="0">
                                </div>
                                <div class="form-group col-md-4">
                                    <label for="minimum_order_price">{{trans('minimum_order_price')}} *</label>
                                    <input type="number"  name="minimum_order_price" value="{{$coupon->minimum_order_price}}"  class="form-control" id="minimum_order_price" min="0">
                                </div>
                                <div class="form-group col-md-4">
                                    <label for="to">{{trans('expire_date')}} *</label>
                                    <input type="text" class="form-control dateTo" name="expire_date" value="{{old('expire_date', $coupon->expire_date)}}" id="expire_date">
                                </div>
                                <div class="form-group col-md-4">
                                    ...
                                    <select name="segmentation_id" id="segmentation_id"  value="{{old('segmentation_id')}}" class="form-control select2 w-100">
                                        ...
                                        @foreach($segmentations as $segmentation)
                                            <option value="{{$segmentation->id}}" {{ old("segmentation_id", $coupon->segmentation_id) == $segmentation->id ? "selected":null}}>{{$segmentation->title}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="form-group col-md-3">
                                    <label class="custom-switch">
                                        <input type="checkbox" name="active" value="1" {{ $coupon->active ? 'checked' : '' }} class="custom-switch-input">
                                        <span class="custom-switch-indicator"></span>
                                        <span class="custom-switch-description">Active</span>
                                    </label>
                                </div>

                                <div class="form-coupon col-md-3">
