                            ...
                            <div class="card-header">
                                <span class="table-add float-right">
                                    @can("create", \App\Models\Category::class)
                                        <a href="{{ route('admin.categories.create') }}" class="btn btn-icon"><i class="fa fa-plus fa-1x" aria-hidden="true"></i></a>
                                    @endcan
                                </span>

                                <span class="table-add float-right">
                                    <a href="{{route('admin.categories.index')}}?view=tree" class="btn btn-icon">
                                        <i class="fa fa-tree fa-1x" aria-hidden="true"></i>
                                    </a>
                                </span>

                                <h4>{{trans('categories_list')}}</h4>
                            </div>
                                    ...
                                    <table class="table table-bordered table-hover mb-0 text-nowrap">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>@if($category->image)<img style="width: 100px" src="{{ asset($category->image) }}">@else - @endif</td>
                                                <td><span class="badge badge-{{ $category->active ? 'success' : 'danger' }}">{{ $category->active?  trans('active') : trans('disabled') }}</span></td>
                                                <td>
                                                    <div class="btn-group dropdown">
                                                        ...
                                                        <div class="dropdown-menu">
                                                            @can("update", $category)
                                                                <a class="dropdown-item has-icon" href="{{ route('admin.categories.edit', ['$category' => $category->id]) }}"><i class="fa fa-edit"></i>{{ trans('edit') }}</a>
                                                            @endcan
